...
class AllClassesGestureRecognizer:
    def __init__(self, model_path, config_path):
        ...
        self.prediction_cooldown = 0.5  # Уменьшено для быстрого отклика
        
        # Кэш для текста (оптимизация отрисовки)
        self.text_cache = {}
        self.last_text_update = 0
        self.text_update_interval = 0.1  # Обновлять текст каждые 100мс
    ...
    def filter_predictions(self, predicted_class, confidence):
        """Фильтрация предсказаний для стабильности"""
        ...
        if len(self.prediction_buffer) >= 5:
            ...
            # Обновляем текущее предсказание если есть консенсус
            if class_counts[most_frequent_class] >= 3:  # Минимум 3 из 5 предсказаний
                gesture_name = self.idx_to_class[most_frequent_class]
    ...
    def run(self):
        ...
        print("Запуск оптимизированного распознавания жестов...")
        ...
        while True:
            ...
            if current_time - self.last_text_update > self.text_update_interval:
                self.text_cache = {
                    'title': "Распознавание жестов (Оптимизировано)",
            ...
            frame = put_russian_text(frame, self.text_cache['gesture'], (10, info_y), 
                                   font_scale=0.8, color=gesture_color, thickness=2)
            info_y += 40
            ...
            cv2.putText(frame, "'q'-выход 'r'-сброс", (10, info_y), 
            ...
            cv2.imshow('Gesture Recognition - All Classes', frame)
...
def main():
    """Основная функция"""
    model_path = 'D:/gesture/all_classes_model_output/all_classes_gesture_model.pth'
    config_path = 'D:/gesture/all_classes_model_output/config.json'
    
    # Проверяем существование файлов
    if not os.path.exists(model_path):
        print(f"Ошибка: Файл модели не найден: {model_path}")
        print("Сначала запустите train_all_classes_model.py для обучения модели")
        return
    ...
    try:
        recognizer = AllClassesGestureRecognizer(model_path, config_path)
        recognizer.run()
    except Exception as e:
